...
$(document).ready(function () {
    $("#OutsideLandDistrictSelected").change(function () {
        var requestTalukaModel = { Districtcode: $("#OutsideLandDistrictSelected").val() };
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#OutsideLandTalukaSelected").empty();
                $.each(data,
                    function (i) {
                        var optionhtml = '<option value="' + data[i].SubDistrictcode + '">' + data[i].SubDistrictname + '</option>';
                        $("#OutsideLandTalukaSelected").append(optionhtml);
                    });
            }
        });
    });
    ...
    $("#OutsideLandTalukaSelected").change(function () {
        var requestVillageModel = { SubDistrictcode: $("#OutsideLandTalukaSelected").val() };
        $.ajax({
            type: "POST",
            url: "/Service/CommonDropdown/GetVillage",
            data: requestVillageModel,
            ...
            success: function (data) {
                $("#OutsideLandVillageSelected").empty();
                $.each(data, function (i) {

                    var optionhtml = '<option value="' + data[i].Villagecode + '">' + data[i].Villagename + '</option>';
                    $("#OutsideLandVillageSelected").append(optionhtml);
                });
            }
        });
...
function ToggleTextBox(add) {
    
    var value = add.split('__')[1]
    if (value == "Istrue") {
        $("#OutsideLandDetail").show();
    }
    else {
        $("#OutsideLandDetail").hide();
    }
