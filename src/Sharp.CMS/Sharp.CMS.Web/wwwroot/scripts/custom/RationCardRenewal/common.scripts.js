$(document).ready(function () {
    ...
    $("#ChangeTypeSelected").change(function () {
        ...
        if (changeTypeValue == 'कुटुंबप्रमुखांमधील बदल') {
            $("#NewMemberDiv").show();
            $("#NewNameDiv").hide();
            $("#NewShopDetails").hide();
            $("#NewAddress").hide();
...
$(document).ready(function () {
    var current_year = new Date().getFullYear();
    $("#NewApplicantDOB").datepicker({
        maxDate: 0, changeYear: true, changeMonth: true, dateFormat: 'dd/mm/yy', yearRange: "1930:current_year", onSelect: function (dateText) {
            CalculateNewApplicantAge();
        }
    });
    $('#NewApplicantAge').prop('readonly', true);

});

$(document).ready(function () {
    var current_year = new Date().getFullYear();
    $("#NewFatherApplicantDOB").datepicker({
        maxDate: 0, changeYear: true, changeMonth: true, dateFormat: 'dd/mm/yy', yearRange: "1930:current_year", onSelect: function (dateText) {
            CalculateNewFatherApplicantAge();
        }
    });
    $('#NewFatherApplicantAge').prop('readonly', true);
...
function CalculateNewApplicantAge() {

    var date1 = new Date();
    var dob = $("#NewApplicantDOB").val();
    var d = dob.split("/");
    var dateTimeInput = new Date(d[1] + "/" + d[0] + "/" + d[2]);
    var date2 = new Date(dateTimeInput);
    ...
    if (pattern.test(dob)) {
        ...
        if (d[2] == "0000") {
            alert("अवैध दिनांक.");
            $("#NewApplicantDOB").val('');
            $("#NewApplicantDOB").focus();

            $("#NewApplicantAge").val('');
            $("#NewApplicantAge").focus();

            return false;
        }
        else {
            $("#NewApplicantAge").val(age);
            $('#NewApplicantAge').prop('readonly', true);
            return true;
        }
    }
        ...
        if (localStorage.getItem('my-lstore') == '1') {
            ...
            $('#NewApplicantAge').prop('readonly', false);
            $("#NewApplicantDOB").val('');
            alert("अवैध दिनांक. कृपया दिनांक (dd/mm/yyyy) मध्ये लिहा.");
        }
        return false;
...
function CalculateNewFatherApplicantAge() {

    var date1 = new Date();
    var dob = $("#NewFatherApplicantDOB").val();
    //alert(dob);
    var d = dob.split("/");
    var dateTimeInput = new Date(d[1] + "/" + d[0] + "/" + d[2]);
    var date2 = new Date(dateTimeInput);
    ...
    if (pattern.test(dob)) {
        ...
        if (d[2] == "0000") {
            alert("अवैध दिनांक.");
            $("#NewFatherApplicantDOB").val('');
            $("#NewFatherApplicantDOB").focus();

            $("#NewFatherApplicantAge").val('');
            $("#NewFatherApplicantAge").focus();

            return false;
        }
        else {
            $("#NewFatherApplicantAge").val(age);
            $('#NewFatherApplicantAge').prop('readonly', true);
            return true;
        }
    }
        ...
        if (localStorage.getItem('my-lstore') == '1') {
            ...
            $('#NewFatherApplicantAge').prop('readonly', false);
            $("#NewFatherApplicantDOB").val('');
            alert("अवैध दिनांक. कृपया दिनांक (dd/mm/yyyy) मध्ये लिहा.");
        }
        return false;
